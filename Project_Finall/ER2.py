...
cols = ['Name', 'efficiency_score', 'Predicted_Turnover'] + [col for col in df_full_output.columns if col not in ['Name', 'efficiency_score', 'Predicted_Turnover']]
...
output_file = f"{base_name}_employee_efficiency.csv"

# Save full dataset
df_full_output.to_csv(output_file, index=False)

# Filter employees predicted to leave and sort by efficiency
...
# Save filtered file
employees_to_leave.to_csv(output_file + "_sorted.csv", index=False)

print("Files created:")
print(f"1. Full dataset saved as: {output_file}")
print("2. Sorted Dataset saved")
