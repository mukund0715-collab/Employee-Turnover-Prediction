...
# Add back employee names for output
output_df = pd.DataFrame({
    'Name': employee_names,
    'efficiency_score': df['efficiency_score'],
    'Predicted_Turnover': df['Predicted_Turnover']
})

# Filter employees predicted to leave and sort by efficiency
employees_to_leave = output_df[output_df['Predicted_Turnover'] == 1]
employees_to_leave = employees_to_leave.sort_values(by='efficiency_score', ascending=False)

# Save to CSV
employees_to_leave.to_csv('employees_to_leave_sorted.csv', index=False)
print("CSV file 'employees_to_leave_sorted.csv' created with employees likely to leave, sorted by efficiency.")
